...
namespace FootballManager.Application.Interfaces.Repositories
{
    public interface ITeamDivisionSeasonRepository
    {
        ...
        Task<bool> ExistsByDivisionSeasonAndTeamIdentityAsync(Guid divisionSeasonId, string teamName, string? teamSuffix, CancellationToken cancellationToken = default);
        Task<List<Guid>> GetTeamIdsAssignedToSeasonAsync(Guid seasonId, CancellationToken cancellationToken = default);
        Task<List<Guid>> GetTeamIdsByDivisionSeasonIdAsync(Guid divisionSeasonId, CancellationToken cancellationToken = default);
