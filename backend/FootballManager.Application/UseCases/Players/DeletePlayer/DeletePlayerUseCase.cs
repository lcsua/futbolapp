...
public sealed class DeletePlayerUseCase : IDeletePlayerUseCase
{
    ...
    public async Task ExecuteAsync(DeletePlayerRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var hasGoalRefs = await _playerRepository.HasGoalReferencesAsync(player.Id, cancellationToken);
        if (hasGoalRefs)
        {
            player.Deactivate();
            player.SoftDelete();
        ...
        {
            await _playerRepository.RemoveAsync(player, cancellationToken);
        }
